...
iperf3 -s
```
(Поскольку по умолчанию у сервера открыт порт 5201, мы можем его поменять, по желанию):
```
iperf3 -s -p 5204
```
...
crontab -e
~~~
### На этом этапе нам дадут выбрать текстовой редактор. Мы выбираем `nano` и пишем туда:
~~~
...
(Чтобы узнать дату нужно прописать `date`)
### Для проверки скрипта заходим в наш ранее созданный каталог
~~~
